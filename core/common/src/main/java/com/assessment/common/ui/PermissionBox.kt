package com.assessment.common.ui
...
/**
 * The PermissionBox uses a [androidx.compose.foundation.layout.Box] to show a simple permission request UI when the provided [permission]
...
fun PermissionBox(
    ...
    val permissionState = rememberMultiplePermissionsState(permissions = permissions) { map ->
        ...
        errorText = if (rejectedPermissions.none { it in requiredPermissions }) {
            ...
            "${rejectedPermissions.joinToString()} required for the MiniPokedexApp"
        }
    }
...
private fun PermissionScreen(
    ...
    if (showRationale) {
        AlertDialog(
            ...
            title = {
                Text(text = "Permissions required by the MiniPokedexApp")
            },
            text = {
                Text(text = "The MiniPokedexApp requires the following permissions to work:\n $permissions")
            },
            confirmButton = {
                TextButton(
                    onClick = {
